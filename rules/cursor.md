...
**This is documentation for setting up Cursor with container-use**

## Configuration

Cursor requires a special MDC (Markdown Configuration) file to enforce container-use rules.
...
2. Create `.cursor/rules/container-use.mdc` with this content:
   ```yaml
   ---
...
3. The `alwaysApply: true` setting ensures these rules are always active