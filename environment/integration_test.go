...
func TestGitTracking(t *testing.T) {
	...
	WithEnvironment(t, "git_tracking", SetupNodeProject, func(t *testing.T, env *Environment) {
		...
		t.Run("CommandAuditLog", func(t *testing.T) {
			...
			// --- Verify: Each command creates a commit ---
			gitLog, err := runGitCommand(ctx, env.Worktree, "log", "--oneline", "-n", "4")
			require.NoError(t, err)
			assert.Contains(t, gitLog, "Run echo 'Running tests", "Command commits should have descriptive messages")
		})
		...
		t.Run("StateRecovery", func(t *testing.T) {
			...
			err := env.Update(ctx, "Checkpoint", "Save production state", env.BaseImage, nil, nil)
			require.NoError(t, err)
			...
			latestNote := strings.Fields(stateNotes)[0]
			noteContent, err := runGitCommand(ctx, env.Worktree, "notes", "--ref="+gitNotesStateRef, "show", latestNote)
			require.NoError(t, err)
			...
			assert.Equal(t, "Checkpoint", env.History[len(env.History)-1].Name, "Latest history entry should be the checkpoint")
		})
		...
		t.Run("CommitMetadata", func(t *testing.T) {
			...
			// --- Verify: Commit has proper author ---
			gitAuthor, err := runGitCommand(ctx, env.Worktree, "log", "-1", "--pretty=format:%an <%ae>")
			require.NoError(t, err)
			assert.Equal(t, "container-use <container-use@dagger.io>", gitAuthor, "Commits should have consistent author")

			// --- Verify: Commit timestamp is reasonable ---
			gitTime, err := runGitCommand(ctx, env.Worktree, "log", "-1", "--pretty=format:%ct")
			require.NoError(t, err)
			commitTime, _ := time.Parse("2006-01-02T15:04:05Z07:00", gitTime)
...
func TestMultipleEnvironmentsRemainIsolated(t *testing.T) {
	...
	WithEnvironment(t, "multiple_environments", SetupPythonProject, func(t *testing.T, env1 *Environment) {
		...
		t.Run("FilesAreIsolated", func(t *testing.T) {
			...
			_, err := env2.FileRead(ctx, "config.dev.json", true, 0, 0)
			assert.Error(t, err, "Dev files should not exist in staging")

			_, err = env1.FileRead(ctx, "config.staging.json", true, 0, 0)
			assert.Error(t, err, "Staging files should not exist in dev")
		...
		t.Run("CommandsAreIsolated", func(t *testing.T) {
			...
			// --- Verify: Isolation ---
			output1, err := env1.Run(ctx, "Check dev", "ls -la | grep -E 'eslint|pm2' || echo 'none'", "/bin/sh", false)
			require.NoError(t, err)
			assert.Contains(t, output1, "eslint", "Dev should have eslint config")
			assert.NotContains(t, output1, "pm2", "Dev should not have pm2 config")
...
func TestSystemHandlesProblematicFiles(t *testing.T) {
	...
	t.Run("PythonCache", func(t *testing.T) {
		...
		WithEnvironment(t, "python_cache", SetupPythonProject, func(t *testing.T, env *Environment) {
			...
			err = env.Update(ctx, "Update", "Continue development", env.BaseImage, nil, nil)
...
// Status: Currently FAILS - exposes caching bug where dag.Host().Directory() returns stale data
func TestWorktreeUpdatesAreVisibleAfterRebuild(t *testing.T) {
	t.Skip("Skipping - demonstrates caching bug where updates are lost after rebuild")
	...
	WithEnvironment(t, "worktree_cache", SetupNodeProject, func(t *testing.T, env *Environment) {
		...
		output, err := env.Run(ctx, "Run v1", "sh script.sh", "/bin/sh", false)
		require.NoError(t, err)
		assert.Contains(t, output, "Version 1", "Should run initial version")
		...
		err = env.Update(ctx, "Rebuild", "Force rebuild", env.BaseImage, env.SetupCommands, nil)
		require.NoError(t, err)
		...
		runOutput, err := env.Run(ctx, "Run v2", "sh script.sh", "/bin/sh", false)
		require.NoError(t, err)
		assert.Contains(t, runOutput, "Version 2", "Should run updated version after rebuild")
		assert.NotContains(t, runOutput, "Version 1", "Should not run old cached version")
	})
}
...
func TestUploadAfterModification(t *testing.T) {
	...
	WithEnvironment(t, "upload_cache", SetupNodeProject, func(t *testing.T, env *Environment) {
		...
		require.NoError(t, err)
		assert.Contains(t, content, "Version 1", "Should upload initial version")
...
func TestWeirdUserScenarios(t *testing.T) {
	...
	t.Run("EnvironmentNameCollisions", func(t *testing.T) {
		te := NewTestEnv(t, "name_collisions")
		te.SetupNodeProject()
		ctx := context.Background()

		// Create first environment
		env1, err := Create(ctx, "My App", te.repoDir, "myapp")
		require.NoError(t, err)
		defer env1.Delete(ctx)

		// Create second environment with SAME name
		env2, err := Create(ctx, "My App", te.repoDir, "myapp")
		require.NoError(t, err)
		defer env2.Delete(ctx)
	...
	t.Run("OrphanedWorktreeRecovery", func(t *testing.T) {
		te := NewTestEnv(t, "orphaned_worktree")
		te.SetupPythonProject()
		ctx := context.Background()

		env, err := Create(ctx, "Test", te.repoDir, "orphan-test")
		require.NoError(t, err)
		...
		env2, err := Create(ctx, "Test", te.repoDir, "orphan-test")
	...
	t.Run("ConfigDirEnvironmentLoss", func(t *testing.T) {
		...
		envs, err := List(ctx, te.repoDir)
		require.NoError(t, err)
		assert.NotContains(t, envs, envID, "Lost environment not in list")

		// Restore config dir
		os.Setenv("CONTAINER_USE_CONFIG_DIR", oldConfigDir)
		env.Delete(ctx) // Clean up with correct config dir
	})

	// t.Run("PartialPropagationFailure", func(t *testing.T) {
	// 	t.Skip("Hard to simulate propagation failure reliably")
	// 	// This would test what happens when propagateToWorktree fails midway
	// 	// Container has changes but worktree doesn't get updated
	// 	// Next operation would see inconsistent state
	// })
}
...
func TestEnvironmentConfigurationPersists(t *testing.T) {
	...
	te := NewTestEnv(t, "config_persistence")
	te.SetupNodeProject()
	...
	t.Run("BaseImagePersists", func(t *testing.T) {
		...
		env, err := Create(ctx, "Test environment", te.repoDir, "alpine-test")
		require.NoError(t, err)
		defer env.Delete(ctx)

		// --- Action: Update to different base image ---
		err = env.Update(ctx, "Switch to Alpine", "Use Alpine Linux", "alpine:latest", nil, nil)
		require.NoError(t, err)
		assert.Equal(t, "alpine:latest", env.BaseImage, "Base image should update")

		// --- Verify: Alpine is actually being used ---
		output, err := env.Run(ctx, "Check OS", "cat /etc/os-release | grep -i alpine || echo 'Not Alpine'", "/bin/sh", false)
		require.NoError(t, err)
		assert.Contains(t, strings.ToLower(output), "alpine", "Should be running Alpine")
	})

	t.Run("SetupCommandsPersist", func(t *testing.T) {
		env, err := Create(ctx, "Test with setup", te.repoDir, "setup-test")
		require.NoError(t, err)
		defer env.Delete(ctx)
		...
		err = env.Update(ctx, "Add tools", "Install development tools", "alpine:latest", setupCmds, nil)
		require.NoError(t, err)

		// --- Verify: Setup commands ran ---
		output, err := env.Run(ctx, "Check setup", "cat /setup.log", "/bin/sh", false)
		require.NoError(t, err)
		assert.Contains(t, output, "Setup complete", "Setup commands should have executed")

		// --- Verify: Tools are available ---
		output, err = env.Run(ctx, "Check curl", "curl --version | head -1", "/bin/sh", false)
		require.NoError(t, err)
	...
	t.Run("EnvironmentVariableLimitations", func(t *testing.T) {
		...
		env, err := Create(ctx, "Test env vars", te.repoDir, "envvar-test")
		require.NoError(t, err)
		defer env.Delete(ctx)
		...
		err = env.SetEnv(ctx, "Configure app", vars)
		require.NoError(t, err)

		// --- Verify: Variables work in current session ---
		output, err := env.Run(ctx, "Check vars", "echo API_URL=$API_URL NODE_ENV=$NODE_ENV PORT=$PORT", "/bin/sh", false)
		require.NoError(t, err)
		assert.Contains(t, output, "API_URL=https://api.example.com", "Variables should be set")
		assert.Contains(t, output, "NODE_ENV=production", "Variables should be set")
		assert.Contains(t, output, "PORT=3000", "Variables should be set")

		// --- Action: Rebuild container ---
		err = env.Update(ctx, "Rebuild", "Rebuild container", env.BaseImage, env.SetupCommands, nil)
		require.NoError(t, err)

		// --- Verify: Environment variables should persist (but currently don't) ---
		output, err = env.Run(ctx, "Check after update", "echo API_URL=$API_URL NODE_ENV=$NODE_ENV PORT=$PORT", "/bin/sh", false)
		require.NoError(t, err)

		// --- Verify: This is what we want - but it currently fails ---
		assert.Contains(t, output, "API_URL=https://api.example.com", "Variables should persist across Update()")
		assert.Contains(t, output, "NODE_ENV=production", "Variables should persist across Update()")
		assert.Contains(t, output, "PORT=3000", "Variables should persist across Update()")
	})

	t.Run("LifecycleOperations", func(t *testing.T) {
		// --- Action: Test Create ---
		env, err := Create(ctx, "Test lifecycle", te.repoDir, "lifecycle-test")
		require.NoError(t, err, "Should create environment")
		require.NotNil(t, env)

		envID := env.ID
		originalWorktree := env.Worktree
		...
		err = env.Update(ctx, "Add Node.js", "Install development tools", "alpine:latest", setupCmds, nil)
		require.NoError(t, err, "Should update with setup commands")

		// --- Verify: Setup command was executed ---
		output, err := env.Run(ctx, "Check Node", "node --version", "/bin/sh", false)
		require.NoError(t, err, "Node should be installed")
		assert.Contains(t, output, "v", "Should show Node version")

		// --- Verify: Worktree location should be stable ---
		assert.Equal(t, originalWorktree, env.Worktree, "Worktree location should not change")
